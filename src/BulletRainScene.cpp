...
BulletRainScene::BulletRainScene()
{
	BulletRainScene::start();
}
...
void BulletRainScene::draw()
{
	...
	if (m_displayUI)
	{
		...
		if (m_debugMode) {
			...
			for (int i = 0; i < 10; i++)
			{
				Util::DrawRect(glm::vec2(m_pBullet[i]->getPosition().x - (m_pBullet[i]->getWidth() / 2.0f), m_pBullet[i]->getPosition().y - (m_pBullet[i]->getHeight() / 2.0f)), m_pBullet[i]->getWidth(), m_pBullet[i]->getHeight());
			}
		}
	}
}

void BulletRainScene::update()
{
	m_move();
	m_pPlanet->update();
	for (int i = 0; i < 10; i++)
	{
		// Call update for every bullet object
		m_pBullet[i]->update();
	}

	// Check collision for every bullet, if collision occurs, play a sound
	for (int i = 0; i < 10; i++)
	{
		if (CollisionManager::circleAABBCheck(m_pPlanet, m_pBullet[i])) {
...
void BulletRainScene::handleEvents()
{
	...
	if (SDL_PollEvent(&event))
	{
		...
		case SDL_MOUSEMOTION:
			...
			m_pPlanet->setPosition(m_mousePosition);
			break;
		case SDL_MOUSEWHEEL:
			wheel = event.wheel.y;
			break;
...
void BulletRainScene::start()
{
	...
	m_pPlanet->setPosition(m_position);
	addChild(m_pPlanet);
...
void BulletRainScene::m_ImGuiKeyMap()
{
	ImGuiIO& io = ImGui::GetIO();
...
void BulletRainScene::m_ImGuiSetStyle()
{
...
void BulletRainScene::m_updateUI()
{
	...
	{
		// Change the scene to "Ball & Brick"
